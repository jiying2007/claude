---
description: "并行施工总控：先深度理解需求并拆解为可执行 CSV TODO，再调度并行子代理分批执行。触发词：并行施工、并行调度、collab、多 agent、parallel、CSV TODO"
---

# Claude 并行施工（Parallel Collab）

> 核心定位：主 Claude 负责深度理解需求与拆解；并行子代理负责按 CSV TODO 执行与回报；主 agent 统一汇总与落地。
...
- **并行调度**：多个 `Agent` 工具调用在同一消息中（`run_in_background: true`）
- **并行跑工具**：在同一响应中多个工具调用
- **落盘写入**：默认主 Claude 串行 `Edit`/`Write`，避免并行写冲突

## 主 Claude 标准执行链（CSV TODO 驱动）

1. **需求澄清**
   - 抽取目标、范围、非目标、验收标准、风险、限制条件
2. **任务包落盘**
   - 目录：`.claude/<timestamp>-<slug>/`
...
2. 思考先行：编码前先拆解与规划
3. Skills 优先：优先使用已存在 slash commands 驱动执行
4. 透明记录：关键决策与变更可追溯
